...
if (isset($_POST['submit'])) {
    ...
    if (empty($userData['address'])) {
        ...
        mysqli_query($Conf, $sqlCode);

        if (!mysqli_query($Conf, $sqlCode)) {
...
$gt = $connection->prepare("INSERT INTO `dar_users` SET `name` = '$nameFamily', `phone` = '$phone', `email` = '$email' , `username` = '$username', `password` = '$password' , `homeAddress` = '$address'");
$gt ->bindValue(1, $nameFamily);
$gt ->bindValue(2, $phone);
$gt ->bindValue(3, $email);
$gt ->bindValue(4, $username);
$gt ->bindValue(5, $password);
$gt ->bindValue(6, $address);
*/ 
/*$gt->execute();*/

    /*if($connection->query($sqlCode) === TRUE){
